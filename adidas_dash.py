...
with col2:
    st.header('Adidas Interactive Dashboard')
...
with col4:
    fig=px.bar(df,x='Retailer',y='TotalSales',labels={"TotalSales": "Total Sales{$}"},
               title='Total Sales by Retailers',hover_data=['TotalSales'],
...
fig3= go.Figure()
fig3.add_trace(go.Bar(x=result1['State'],y=result['TotalSales'],name='Total Sales'))
fig3.add_trace(go.Scatter(x=result1['State'],y=result1['UnitsSold'], mode='lines',
                          name='Units Sold',yaxis='y2'))

fig3.update_layout(
    title='Total Sales & Units Sold by State',xaxis=dict(title='State'),
    yaxis=dict(title='Total Sales',showgrid=False),yaxis2=dict(title='Units Sold',overlaying='y', side='right'),
...
with dwn3:
    st.download_button("Download data",data= result1.to_csv().encode('utf-8'),file_name='Sales by units sold.csv',
                       mime='text/csv')
...
fig4=px.treemap(treemap, path=['Region','City'],values='TotalSales',
                hover_data=['TotalSales(Formatted)'], hover_name='TotalSales(Formatted)',
...
_, view5, dwn5 =st.columns([0.1,0.45,0.45])
with view5:
    expander=st.expander('View Sales Raw Data')
    expander.write(df)
with dwn5:
    st.download_button('Download data', data=df.to_csv().encode('utf-8'),
                       file_name='Sales data.csv',mime='text/csv')
st.divider()
